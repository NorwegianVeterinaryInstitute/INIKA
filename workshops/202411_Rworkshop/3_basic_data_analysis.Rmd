...
they are excluded, so we need to correct this). Type F1 for help. 
...
#### Bonus - formatting the summary table and export in a file.
...
```{r}
format_summary_statistics_fun(human_data_selection) %>%
  View()
```
...
variables (including identifiant) - raw : observations
...
NB: A package that I find practical to use (but you can also search on
