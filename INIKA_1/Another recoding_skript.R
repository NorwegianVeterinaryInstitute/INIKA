...
QUEST<-Test%>%
  rename( Respondent= "Name of the respondent",
          ...
          Manage_Healthy_Chicken= "What do you do to keep your chicken healthy, so they don't get sick? (don't read options)"  ,                                                                                                   
          ...
          Manage_Healthy_Chicken_Specification= "If others, specify...122"  ,
  ...
  mutate(Status_R = case_when(
    Status_R == "Maried" ~ "Married",
    Status_R == "Window" ~ "Widow",
    TRUE ~ Status_R
  )) %>%
  mutate(Status_F = case_when(
    Status_F == "Maried" ~ "Married",
    Status_F == "Window" ~ "Widow",
    TRUE ~ Status_F
  )) %>%
  mutate(Status_F_2 = case_when(
    Status_F_2 == "Maried" ~ "Married",
    Status_F_2 == "Window" ~ "Widow",
    TRUE ~ Status_F_2
  )) %>%
  ...
  mutate(Ethnic_R = case_when(
    Ethnic_R %in% c("MCHAGA", "Mchaga") ~ "Chaga",
    Ethnic_R == "Mpare" ~ "Pare",
    Ethnic_R == "Msukuma" ~ "Sukuma",
    TRUE ~ "Others"
  )) %>%
  mutate(Ethnic_F = case_when(
    Ethnic_F %in% c("MCHAGA", "Mchaga") ~ "Chaga",
    Ethnic_F == "Mpare" ~ "Pare",
    Ethnic_F == "Msukuma" ~ "Sukuma",
    TRUE ~ "Others"
  )) %>%
  mutate(Education_F = case_when(
    ...
    Education_F %in% c("Degree", "Diploma", "Advanced diploma", "Bachelor", "Cerrificate", "Certificate - Animal Health") ~ "College",
    TRUE ~ Education_F
  )) %>%
  ...
  mutate(AB_Understanding_New = case_when(
  ...
  mutate(Breedkat = case_when(
    Breed %in% c(
      "Alvin", "Arvin broiler", "Arvin broilersBroiler", "Broiler - arvin",
      "Broiler -arvin", "Broiler and croiler", "Broiler and Saso", "Broiler arvin",
      "Broiler Ross 308", "Broiler silverland", "Broiler, Kuroirer and Saso",
      "Broiler, layers", "Broiler, saso", "Broiler,alvin", "Broiler,layers",
      "Broiler,Saso", "Broilers alvin", "Cob 500", "Cobb 500", "Cobb and Rose",
      "Cobb500", "cobb500 & sasso", "Conb500Roos 308", "Ros308", "Ross 305",
      "Ross 308", "Ross300", "Ross308", "Intercheeck broiler", "Interchick",
      "Interchicks", "Interchicks and centrachicks", "Interchicks and centrochicks",
      "Saso and broiler", "Silverland broiler", "Saso and Broiler", "Saso Ross",
      "Saso Ross 308", "Saso silverland", "Saso silverland 308", "Silverland broiler",
      "Saso, kroiler and Broiler",
      "300", "Arvin broilers", "Broiler", "Conb500",
      "Roos 308"
    ) ~ "Broiler",
    Breed %in% c(
      "Chroirer", "Tanbro,saso", "Croiler", "Croiler - Interchicks", "Croiler and Saso", "Asho", "Ashok", "Ashork", "Bovans", "Chotara", "Erveness and sendlecheck",
      "Evines", "Falcon", "hashokh", "Intercheeck ,and kencheek", "Joshi",
      "Kibo and silverland", "Kloiler", "Kroiler", "Kroiler/ layers", "Kuroiler",
      "Layers and saso", "Saso", "Saso- silver land", "Saso- tanbro", "Saso - silverland",
      "Saso & kloiler", "Saso 38", "Saso& kroiler", "Saso, croiler",
      "Saso,chotara", "Saso,Chotara", "Silverland", "Silverland saso", "Tanbro",
      "Tanbro -intercheeck", "Tanbro and Saso", "Tanbro,sasoall", "Crossbred"
    ) ~ "Crossbred",
    TRUE ~ Breed
  ))